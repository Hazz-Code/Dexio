  ...
  return listings.map(item => {
    ...
    return (
      <div key={item.id} style={{
        ...
        <div style={{ fontSize: 28, background: sticker.bg, borderRadius: 10, padding: '6px 8px' }}>
          {sticker.emoji}
        </div>
        <div style={{ flex: 1 }}>
        ...
        <button
          onClick={() => !alreadyOwned && onOffer(item.profile, sticker)}
          style={{
            background: alreadyOwned ? 'rgba(255,255,255,0.05)' : 'rgba(124,58,237,0.15)',
            border: `1px solid ${alreadyOwned ? 'rgba(255,255,255,0.06)' : 'rgba(124,58,237,0.3)'}`,
            borderRadius: 10, padding: '6px 10px',
            color: alreadyOwned ? '#334155' : '#A78BFA',
