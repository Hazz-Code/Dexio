...
export function StickerCard({ sticker, mini, owned = true, forTrade, onClick }) {
    ...
    >
      {forTrade && (
        <div style={{
          ...
          fontSize: 8, fontWeight: 700, padding: '2px 4px', borderRadius: 10,
        }}>TRADE</div>
      )}
      <div style={{ fontSize: 26 }}>{sticker.emoji}</div>
  ...
  return (
    <div
      ...
      onMouseLeave={e => e.currentTarget.style.transform = 'scale(1)'}
    >
      <div style={{ fontSize: 52 }}>{sticker.emoji}</div>
      ...
      {forTrade && (
        <div style={{ background: '#F59E0B', color: '#000', fontSize: 10, fontWeight: 700, padding: '3px 10px', borderRadius: 20 }}>
          FOR TRADE
        </div>
      )}
    </div>
